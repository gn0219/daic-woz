...
with open('results/tda_result/classification_results.txt', 'w') as f:
    f.write("Random Forest Classification Results (1D Persistence Images):\n\n")
    f.write(classification_report(y_test, y_pred))
    f.write(f"\nAccuracy: {accuracy_score(y_test, y_pred):.4f}\n")
    f.write(f"AUROC: {roc_auc_score(y_test, y_pred_proba):.4f}\n")
    f.write(f"F1-macro: {f1_score(y_test, y_pred, average='macro'):.4f}\n")


cm = confusion_matrix(y_test, y_pred)
# Plot confusion matrix using custom plot_cm function
plot_cm(cm, class_names=['Non-depressed', 'Depressed'], 
        title='Confusion Matrix - TDA with Random Forest',
        save_path=os.path.join('results/tda_result', 'confusion_matrix.png'))
